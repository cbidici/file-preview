...
function Preview({fileId, path}) {
    ...
    const preview = function () {
        ...
        if(file.type.startsWith("IMAGE")) {
            return <img hidden={isPreviewLoading} alt="" src={process.env.REACT_APP_FILE_SERVICE_HOST+'/previews/'+file.id} onLoad={onImageLoad} onError={onImageLoad} />;
        } else if(file.type.startsWith("VIDEO")) {
            return <video display={isPreviewLoading ? "none" : "block"} ref={videoRef} id="" controls autoPlay onCanPlay={onImageLoad} onEmptied={onImageLoad} playsInline webkit-playsinline="true">
    ...
    return (
        <div className="modal show" id="imageModal" tabIndex="-1" style={{display: 'block', backgroundColor: 'rgba(0, 0, 0, 1)'}}  onClick={() => dismissPreview()}>
          <div className="modal-dialog modal-xxl">
            <div className="modal_preview_container d-flex align-items-center" onClick={e => e.stopPropagation()}>
              ...
              <div className="modal_preview_element">
                  {preview()}
              </div>
              {isPreviewLoading &&
