...
function Thumbnail({resource}) {
  ...
  if(resource.type === 'DIRECTORY') {
    ...
    thumbnail =
      <button type="button" disabled={isThumbnailLoading} onClick={() => preview(resource.id)}>
    ...
    thumbnail =
      <button type="button" disabled={isThumbnailLoading} onClick={() => preview(resource.id)}>
        <svg xmlns="http://www.w3.org/2000/svg" width="50" height="50" fill="currentColor" className="bi bi-play-circle play" viewBox="0 0 16 16">
  ...
  return (
    <div>
      ...
      <div className='card-body pb-2 name_container'>{resource.name}</div>
    </div>
  );
}
